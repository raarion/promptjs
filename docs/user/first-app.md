...
> **CSS scoping tersedia sebagai opt-in.** Secara default `Gaya:` tetap GLOBAL (tidak ada perubahan perilaku untuk proyek lama). Tambahkan front-matter `gayaCakupan: benar` untuk mengaktifkan isolasi style per-komponen: setiap elemen mendapat atribut `data-pjs-<namafile>-<namakomponen>` (atau `data-pjs-<namafile>` untuk style top-level halaman), dan selector CSS otomatis ditulis ulang agar cocok. Detail: [LIM/MIS Mapping v132 § CSS scoping design decision](../project/lim-mis-mapping-v132.md#css-scoping-design-decision-79). Progres: [GitHub issue #79](https://github.com/raarion/promptjs/issues/79).
>
> **CSS scoping is available as an opt-in.** By default `Gaya:` remains GLOBAL (no behavior change for existing projects). Add the front-matter directive `gayaCakupan: benar` to enable per-component style isolation: every element gets a `data-pjs-<fileName>-<componentName>` attribute (or `data-pjs-<fileName>` for page-level top-level styles), and CSS selectors are automatically rewritten to match. Details: [LIM/MIS Mapping v132 § CSS scoping design decision](../project/lim-mis-mapping-v132.md#css-scoping-design-decision-79). Tracking: [GitHub issue #79](https://github.com/raarion/promptjs/issues/79).
