...
// [DX-1 FIX] Engine sets this when the parser hit a fatal token error (E2020);
// suppresses the misleading E3001 undeclared-identifier cascade. Declared so TS
// is aware of the dynamic patch hook.
/** @type {boolean} */
...
PromptJSResolver.prototype.visitIdentifier = function (node) {
  ...
  if (symbol) {
    ...
    // [DX-1 FIX] Suppress the misleading E3001 cascade when the parser already
    // reported an unrecoverable token error (E2020). The partial AST contains
    // stray identifiers (often tag names like `span`) that are NOT real
    // undeclared-variable bugs, so blaming them just buries the real syntax
    // error under noise.
...
PromptJSResolver.prototype.visitAmbilLuarStatement = function (node) {
  ...
  // [BUG-3 FIX] Inline fetch bind form `ambil dari "url" ke <target>` writes to
  // the bound state AND its companion auto-state flags via `__setState(...)` in
  // the emitter (`<target>`, `<target>_memuat`, `<target>_galat`). These uses
  // are IMPLICIT (generated by codegen, not written in the DSL), so without
  // this the analyzer wrongly reports them as W4101 "declared but never used"
  // and, worse, suggests deleting the loading/error state the UI needs.
  // Mark all three as both read and written so no unused/dead-write warning
  // fires for auto-state that the fetch demonstrably drives.
  if (node.bindTarget) {
 ...
 * @returns {void}
 */
PromptJSResolver.prototype._markFetchAutoStateUsed = function (name, node) {
  if (!name) return;
...
PromptJSResolver.prototype.visitUlangiStatement = function (node) {
  ...
  if (node.transitionName && !node.keyExpr) {
    // [DX-2 FIX] Previously the transition was deleted SILENTLY, leaving the
    // developer to wonder why `dengan transisi <name>` produced no animation.
    // Keep the honest-keyword drop, but surface a W3004 warning so the reason
    // (missing `dengan kunci`) is visible instead of a mysterious no-op.
...
PromptJSResolver.prototype.visitKetikaStatement = function (node) {
  ...
  if (node.event && typeof node.event === 'string' && !VALID_EVENT_NAMES.has(node.event)) {
    // [INK-1 FIX] An unknown event name is intentionally NON-FATAL: the emitter
    // still binds `addEventListener(<event>, ...)` so custom/synthetic events
    // remain a valid escape hatch. The diagnostic therefore belongs in the
    // `warnings` bucket AND must carry `severity: 'warning'` so bucket and
    // severity are consistent (previously E4009 sat in warnings but reported
    // severity 'error', contradicting `success: true`). Code + message kept for
    // backward compatibility with existing tooling/tests.
    this.warnings.push(
